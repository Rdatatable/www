...
<p > <span class="text-info"><span class="glyphicon glyphicon-info-sign"></span></span> All benchmark timings reported are the **minimum of three consecutive runs**. Remember, these are large data benchmarks; they necessarily take a long time to run (many minutes for a single run, typically). </p>
</div>
<div class='bs-docs-section'>
...
<p > We'll consider aggregation/grouping operations in this section. The equivalent function for `dplyr` is <code4>summarise()</code4>, which summarises each group into a single row. Alternatively, <code4>summarise_each()</code4> is used to summarise multiple columns. </p>
...
<p > `dplyr` *doesn't* like modifying objects *in-place*, whereas it is a very important aspect of `data.table's` philosophy. This is also one of the major philosophical differences between the two packages. </p>
...
<p class='lead'> In this section, we'll benchmark `data.table` <code4>joins</code4> against `dplyr`'s. For now, We'll consider only <code4>left join</code4> for now. We will update with the other joins soon. </p>
...
<p class='lead'> `data.table 1.9` imports the package <code4>reshape2</code4> and implements `data.table` specific methods for  both <code4>melt</code4> and <code4>dcast</code4> in <code>C</code>. It's both fast and memory efficient. We'll compare them here to with `reshape2`'s respective methods. </p>
...
<p > We'll benchmark on <code4>1e8 rows</code4>. Also, we'll compare <code4>melt</code4> with and without the argument <code4>na.rm</code4>. </p>
...
<p > Let's benchmark on <code4>1e8 rows</code4>. Columns <code4>aa</code4> and <code4>bb</code4> both have 1e2 and 1e3 unique values each. </p>
<!-- dcast table -->
